...
void drawPanel() {
  ...
  int lx=sx+610,ly=sy+14;
  drawLegendDot(lx,    ly,color(185,155,105),"Pó de cal");
  drawLegendDot(lx+95, ly,color(220,200,140),"Zona ativa");
  drawLegendDot(lx+195,ly,color(170,120,70), "Caindo");
  drawLegendDot(lx+270,ly,color(100,60,20),  "Incrustação");
  drawLegendDot(lx+370,ly,color(160,200,255),"Onda acústica");
  ...
  String[] vals={nf(sliderParticleRate,0,1)+" frames",nf(sliderGapTime,0,1)+" s",nf(sliderBuzzTime,0,1)+" s",nf(sliderHornRange*100,0,0)+"% do canal"};
  for (int i=0;i<4;i++) {
